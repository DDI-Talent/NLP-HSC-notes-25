...
Corona_DF <- read_csv("data/CORONA_TWEETS.csv")
...
To access a specific column in names command, use the index/position of the column. For example

`names(CoronaDF)[5]` means the column name `TweetAt` (column number 5) in the data frame `Corona_DF.`

```{r}
Corona_DF <- read.csv("data/CORONA_TWEETS.csv")

# you can rename one column:
names(Corona_DF)[5]<-"TweetDate" # rename TweetAt to TweetDate
glimpse(Corona_DF)
```
